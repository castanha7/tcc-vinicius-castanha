...
// ARMAZENAMENTO EM MEMÓRIA DAS OPORTUNIDADES E ESTADO DO USUÁRIO
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      try {
        // Verifica a Custom Claim de administrador
...
// SISTEMA DE NAVEGAÇÃO DE TELAS COM PROTEÇÃO DE ROTA
window.navigateTo = function(viewName, param = null) {

  // Usuários que não são administradores não podem acessar o Painel Admin
...
function updateAuthUI(user) {
  ...
  // Localiza os botões que levam ao Painel Admin
  ...
  if (user) {
    ...
    // Identificação visual do administrador
    if (navUsername) {
    ...
    // Mostra o botão Admin somente para administradores
    adminNavBtns.forEach(btn => {
      btn.style.display = isAdmin
        ? 'inline-block'
        : 'none';
    });
    ...
    // Visitantes não veem o botão Admin
    adminNavBtns.forEach(btn => {
...
function setupFormListeners() {
  ...
  if (oppForm) {
    ...
    oppForm.addEventListener('submit', async (e) => {
      ...
      if (!isAdmin) {
        alert(
          'Operação negada: Permissão de administrador requerida.'
        );
        return;
      }
      ...
      const title =
        document.getElementById('oppTitle').value.trim();
...
window.openNewOpportunityModal =
  function() {
    ...
    if (!isAdmin) {
      alert(
        'Operação negada: Permissão requerida.'
      );
      return;
    }
